...
String[] arrTipeLayanan = tipeLayanan.split(GlobalVariable.Separator)
for (int i = 0; i < arrTipeLayanan.length; i++) {
	TestData data = findTestData("Data Files/Muatmuat - Apps/tipe_layanan")
	for (int j = 1; j <= data.getRowNumbers(); j++ ) {
		if (arrTipeLayanan[i] == data.getValue(1, j)) {
