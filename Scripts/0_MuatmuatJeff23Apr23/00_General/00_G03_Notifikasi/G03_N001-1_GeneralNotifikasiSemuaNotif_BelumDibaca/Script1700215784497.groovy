...
if (index > -1) {
    hasilCekNotif.add('V Berhasil Ganti View Notif Semua Belum Dibaca')
} else {
    boolean noNotifPresent = WebUI.verifyTextPresent('Anda Belum Menerima Notifikasi', false, FailureHandling.OPTIONAL)

    if (noNotifPresent) {
        hasilCekNotif.add('V Berhasil Ganti View Notif BF Semua Dibaca dan Kosong Benar')
    } else {
        hasilCekNotif.add('X Gagal Ganti View Notif Semua Belum Dibaca')
    }
}
