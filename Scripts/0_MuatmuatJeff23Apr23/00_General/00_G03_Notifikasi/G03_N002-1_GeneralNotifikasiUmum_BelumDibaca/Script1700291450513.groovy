...
WebUI.click(findTestObject('0_MuatmuatJeff23Apr23/00_General/Page_Notifikasi  muatmuat/div_Umum'))
...
if (index > -1) {
    hasilCekNotif.add('V Berhasil Ganti View Notif Umum Belum Dibaca')
} else {
	boolean noNotifPresent = WebUI.verifyTextPresent('Anda Belum Menerima Notifikasi', false, FailureHandling.OPTIONAL)
	
	if (noNotifPresent) {
		hasilCekNotif.add('V Berhasil Ganti View Notif BF Umum Dibaca dan Kosong Benar')
	} else {
		hasilCekNotif.add('X Gagal Ganti View Notif Umum Belum Dibaca')
	}
}
